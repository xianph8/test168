...
//        if (BuildConfig.DEBUG) Log.d(TAG, "uuid=" + uniqueId);
    ...
    /**
     * 获取当前设备唯一标识码
     * 注意，如果是android M 设备，那就需要向用户申请该权限，否则崩溃
     *
     * @param context
     * @return 设备唯一标识码
     */
...
//android.permission.READ_PHONE_STATE
        id = tm.getDeviceId();
        if (null != id) {
            Pattern pattern = Pattern.compile("^[0]{0,}$", Pattern.CASE_INSENSITIVE);
            Matcher matcher = pattern.matcher(id);
//            Log.d(TAG, "device id ：" + matcher.matches());
            if (!matcher.matches()) {///////////id can't be null and all zero
                Log.d(TAG, "device id ：" + id);
                return id;
            }
        }

        id = "35"  // make this look like a valid IMEI
        ...
        if (null != id) {
            Log.d(TAG, "make id ：" + id);
            return id;
        }
        return "";
    }

    public static String getAppTempPath(Context mContext) {
        return FileUtils.getExternalStoragePath(mContext) + "temp" + File.separator;
    }

    public static String getAppDownloadPath(Context mContext) {
        return FileUtils.getDownloadDir(mContext) + File.separator;
    }
